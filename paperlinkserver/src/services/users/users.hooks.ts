...
const rewardReferralCredit = async (context: any) => {
  ...
  if (context.data.referreeCode) {
    ...
    const referree: any = await app.services.users.find({
      query: { referralCode: context.data.referreeCode }
    })
    let regLeaves: any = await app.services['admin-settings'].find({
      query: { name: DEFAULT_VALUES.adminSettings.REGISTRATION_LEAVES }
    })
    if (referree.total > 0) {
      ...
      await app.services.users.patch(referree.data[0].id, {
        totalLeavesEarned: referreeTotalLeaves,
        numberOfReferrals: numberOfReferrals
      })

      //create notification
      const notificationSettings: any = await app.services['notification-settings'].find({
        query: { userId: referree.data[0].id }
      })
      let message =
        '{' +
        ...
        '}' +
        ' ' +
        DEFAULT_VALUES.notificationMessage.SUCCESSFULLY_REFERRAL_SIGNUP +
        'you earn ' +
        regLeaves[0].value +
        ' leaves'
      ...
      let disperseLeaves: any = await app.services['admin-settings'].find({
        query: { name: DEFAULT_VALUES.adminSettings.TOTAL_LEAVES_DISPERSED }
      })
...
const joinATeam = async (context: any) => {
  if (context.data.teamId) {
    ...
    if (team.role === DEFAULT_VALUES.users.roles.PAID_USER) {
      ...
      const checkIfAlreadyRegistered: any = await app.services.users.find({
        query: { teamId: context.data.teamId, mainAccountId: context.data.userId }
      })
      ...
      if (context.data.teamId != user.id) {
        ...
        ;(context.data.status = DEFAULT_VALUES.users.status.PENDING), (context.data.mainAccountId = user.id)
        context.data.teamAccess = DEFAULT_VALUES.membersAccess.OWN_FILES
        context.data.referralCode = null

        //create notification
        const appNotificationSettings: any = await app.services['notification-settings'].find({
          query: { userId: context.data.teamId }
        })
            ...
            if (notification.inviteRequest === true) {
              await app.services.notification.create({
                message:
                  ...
                  user.lastName +
                  '} ' +
                  DEFAULT_VALUES.notificationMessage.SUCCESSFULLY_JOIN_A_TEAM +
                  ' {' +
                  team.companyName +
                  '}',
                userId: team.id
...
const createUsersDefaultSetup = async (context: any) => {
  if (
    context.data.action === DEFAULT_VALUES.users.FORGOT_PASSWORD ||
    context.data.action === DEFAULT_VALUES.users.RESET_PASSWORD
  ) {
    return context
  }
  try {
...
const forgotPassword = async (context: any) => {
  if (context.data.action === DEFAULT_VALUES.users.FORGOT_PASSWORD) {
    ...
    let link = process.env.APP_URL + `/reset-password/?token=${token}`
...
const addBusinessPage = async (context: any) => {
  if(!context.data.action && context.data?.companyName){
    let page: string = context.data?.companyName?.replace(/\s+/g, '') || ''
    const checkForDuplicate: any = await app.services.users.find({
      query: { businessPage: page }
    })
    context.data.businessPage = checkForDuplicate.total === 0 ? page : `${page}${checkForDuplicate.total}`
  }
  return context
}

const updatePassword = async (context: any) => {
  if (context.data.action === 'user_update_password' && context.data.password) {
  ...
  }else if(context.data.action === 'team_member_update_password' && context.data.password) {
   return context
  }
  return context
}
