...
export default Vue.extend({
  ...
  methods: {
   async loginFromExternalWebsite(){
      let externalToken = this.$route.query.paperlinkAccessToken as string
      let userId = this.$route.query.id as string
      if(!externalToken) return
      (this.$auth.strategy as any).token.set(externalToken)
     try {
