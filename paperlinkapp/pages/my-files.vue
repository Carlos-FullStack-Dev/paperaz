<template>
  <main class="mb-24">
    ...
    <div class="flex items-center flex-wrap gap-4 justify-between mb-6 relative">
      ...
      <table class="custom-table" v-else>
        ...
        <tbody class="text-[#505050]">
          <tr v-for="(file, i) in pdfUser" :key="i">
            <td class="w-12 text-center fixed-col left">
              {{ i + 1 + returnedDataPage }}
            </td>
            <td>
              <div class="flex items-center gap-3">
                <span class="circle circle-17 p-0.5 border border-paperdazgreen-400">
                  <img
                    :src="
                      (file.user || {})?.profile_picture || '/img/placeholder_picture.png'
                    "
                    alt=""
                    class="circle"
                  />
            ...
            <td class="text-center">
              <div class="grid place-items-center">
                <button class="circle circle-17 bg-paperdazgreen-400 text-white">
                  ...
                  <el-dropdown-menu slot="dropdown" class="table-menu-dropdown-menu">
                    <ul class="min-w-[150px]">
                      <li class="dropdown-item">
                        <span @click="showShareCompanyFileFunc(file)">Share</span>
                      </li>
...
export default Vue.extend({
  ...
  methods: {
    ...
    async fetchFiles(page, search) {
      await this.$axios
        .$get(
          `/files?userId=${this.$auth.user.id}&fileName[$like]=${search}%&$skip=${page}&$sort[updatedAt]=-1`,
          {
            params: {
              isEditing: 0,
            },
    ...
    formatDateTime(dateVal) {
      return `${DateFormatter.getDateString(dateVal)}  ${DateFormatter.getFormattedTime(
        dateVal
      )}`;
    },
  },
