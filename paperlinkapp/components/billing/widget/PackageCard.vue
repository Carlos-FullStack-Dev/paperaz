<template>
  ...
  >
    ...
    <div class="px-2 lg:px-5 py-4">
      ...
      <div
        class="lg:border-t-2 border-paperdazgreen-400 mb-5 text-[1rem] font-[500] flex justify-between"
      >
        <span>Total Amount</span>
        <span class="text-[#606060]">{{ totalPrice }}</span>
      </div>
      <ul class="package-list">
        ...
        <li class="">
          <span><tick-icon width="20" height="20" /></span>
          <span>{{ (stagingPackage || {}).fillable }} Fillable PDF</span>
        </li>
        <li class="">
          <span :class="[stagingPackage.whiteGlove ? 'opacity-100' : 'opacity-0']"><tick-icon width="20" height="20" /></span>
...
export default Vue.extend({
  ...
  methods: {
    async setPackageData() {
       await this.$axios.patch(`subscriptions/${this.$auth.user.subscriptionId}`, {
        action: 'buyMoreFeature',
        additional_paperlink: this.stagingPackage.paperlink,
        additional_teamMembers: this.stagingPackage.teamMembers,
      })
      // return
