<template>
  <div>
    <transition name="fade" mode="out-in">
      ...
      <form class="" v-if="displayingForm == 'register'" @submit.prevent="register">
        <legend class="block text-center font-semibold mb-6">Signup to Paperdaz</legend>
        ...
        <div class="form-group">
          ...
          <small
            v-show="
              formData.password && confirmPassword && confirmPassword != formData.password
            "
            class="text-red-500"
      ...
      <!-- Start:: login form -->
      <form class="" v-else-if="displayingForm == 'login'" @submit.prevent="login">
        <legend class="block text-center font-semibold mb-6">Login to Paperdaz</legend>
      ...
      <form class="" v-else-if="displayingForm == 'confirm'" @submit="updateUserRecords">
...
export default mixins(login).extend({
  ...
  computed: {
    ...
    displayingForm() {
      if (!this.$auth.loggedIn)
        this.$nuxt.$router.push(`/register?hasPackageId=${this.setPackage.id}`);
  ...
  methods: {
    async confirmCompanyEmailInDatabase() {
      await this.$axios
        .get(`/users/?email=${this.confirmFormData.companyEmail}`)
        .then((response) => {
