<template>
  <section class="relative">
    ...
    <div class="rounded-2xl bg-white overflow-x-auto custom-scrollbar p-6 relative">
      ...
      <!-- << -- spinner contaier --<< -->
      <div
        v-if="referalsIsLoaded"
        class="absolute w-full top-0 left-0 h-full z-10 bg-white flex justify-center items-center"
      >
...
export default Vue.extend({
  ...
  mounted() {
    ...
    this.referalLink = `${window.location.origin}/register/?referreeCode=${this.$auth.user.referralCode}`;
  ...
  methods: {
    ...
    async getAllReferee(page) {
      await this.$axios
        .$get(
          `/referral/?referralCode=${this.$auth.user.referralCode}&$sort[id]=-1&$skip=${page}`
        )
        .then((response) => {
