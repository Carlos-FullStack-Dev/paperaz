<template>
  ...
  >
    ...
    <!--End:: Close Button -->
    <template #title>
      <h4 class="text-center font-semibold text-xl">Edit Folder Name</h4>
    </template>
    <!-- Start:: Body -->
    <p class="text-center block font-medium mx-auto mb-6 whitespace-none">
      <input
        v-model="folderInputData"
        class="w-full py-3 px-2 border-[1px] border-paperdazgrey-500 rounded-md"
        placeholder="Enter Folder Name"
      />
    </p>
    <div class="flex justify-around">
      <button
        class="disabled:bg-opacity-50 disabled:cursor-progress h-10 text-xs w-[900%] shadow-md text-white rounded-lg bg-paperdazgreen-400"
        :disabled="loading"
        @click="onSubmit"
      >
