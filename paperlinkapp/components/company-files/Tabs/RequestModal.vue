<template>
  ...
  >
    ...
    <form ref="form" @submit.prevent="onSubmit">
      ...
      <p class="text-center block font-medium mx-auto mb-4 whitespace-none">
        <textarea
          required
      ...
      <div class="flex justify-around">
        <button
          class="disabled:bg-opacity-50 disabled:cursor-progress h-10 text-xs w-[900%] text-white rounded-xl shadow bg-paperdazgreen-400"
          :disabled="loading"
        >
  ...
  methods: {
    ...
    addEmailinputs() {
      ...
      Array.from(this.$refs.form.elements).forEach((element) => {
        if (element.type == "email" && !element.value.trim()) addEmail = false;
      });
      if (!addEmail) return;
