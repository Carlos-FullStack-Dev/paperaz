<template>
  ...
  >
    ...
    <!-- Start:: Body -->
    <p class="text-center block font-medium max-w-[290px] mx-auto mb-0 whitespace-none">
      You want to remove
      <div class="font-semibold text-paperdazgreen-500 uppercase text-center mb-2 ">
        {{
                    `${member?.user?.firstName || "Pending"} ${
                      member?.user?.lastName || ""
                    }`
                  }}
      </div>
      <div class="font-semibold text-paperdazgreen-500 uppercase text-center mb-6 ">
        {{ member?.user?.email }}
      </div>
    </p>
