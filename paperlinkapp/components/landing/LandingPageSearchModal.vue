  ...
  methods: {
    ...
    remoteSearch() {
      ...
      Promise.all([files, company, user]).then((response) => {
        ...
        let userArr = [];
        let user_id = localStorage.getItem("paperdaz_userID");
        
        //search business page url

        let user_business_page = "";
        userArr = response[2].data;
        userArr.forEach(item => {
          if(item.id == user_id) {
            user_business_page = item.businessPage;
          }
        });
        localStorage.setItem("businessPage_url", user_business_page);
